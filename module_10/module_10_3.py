...
class Bank:
    ...
    def deposit(self):
        for i in range(100):
            rendom1 = randint(50, 500)
            self.balance = self.balance + rendom1
            print(f"Пополнение: {rendom1}. Баланс: {self.balance}")
    ...
    def take(self):
        for e in range(100):
            rendom2 = randint(50,500)
            print(f"Запрос на {rendom2}")
            if self.balance >= rendom2:
                self.balance = self.balance - rendom2
                print(f"Снятие: {rendom2}. Баланс: {self.balance}")
            else:
...
По итогу вы получите скрипт разблокирующий поток до баланса равному 500 и больше или блокирующий, когда происходит попытка снятия 
